...
*Setup instructions for a Gitian build of FTCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FTCoin
...
- [Building FTCoin Core](#building-ftcoin-core)
...
Clone the git repositories for FTCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/TFTcoin/ftcoin
```
...
in the FTCoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build FTCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the FTCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ftcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/TFTcoin/ftcoin
...
For example:
```bash
URL=https://github.com/crowning-/ftcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ftcoin=${COMMIT} --url ftcoin=${URL} ../ftcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ftcoin=${COMMIT} --url ftcoin=${URL} ../ftcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ftcoin=${COMMIT} --url ftcoin=${URL} ../ftcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ftcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ftcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ftcoin.git
SIGPATH=/some/root/path/ftcoin-detached-sigs.git

./bin/gbuild --url ftcoin=${BTCPATH},signature=${SIGPATH} ../ftcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ftcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ftcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ftcoin-osx-build.assert
```
...
[ftcoin/gitian.sigs](https://github.com/tftcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.

...
const char * const BITCOIN_CONF_FILENAME = "ftcoin.conf";
const char * const BITCOIN_PID_FILENAME = "ftcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ftcoin" is a composite category enabling all FTCoin-related debug output
            if(ptrCategory->count(string("ftcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FTCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FTCoinCore
    // Mac: ~/Library/Application Support/FTCoinCore
    // Unix: ~/.ftcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FTCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FTCoinCore";
#else
    // Unix
    return pathRet / ".ftcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ftcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ftcoin.conf
